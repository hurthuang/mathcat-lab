...
- **比例（`a:b`）MathCAT 沒有特化成 Rule 21.2 專屬的比例記號**：`x:y=2:1` 這種輸入，nc
  給 ⠐⠂（Rule 21.2 比例記號），MathCAT 給 ⠸⠒（一般 Rule 8.5 冒號，跟時間冒號同一個符號）；
  另外測了單獨一個裸 `:`，MathCAT 又給第三種結果 ⠒，三次輸出彼此都不同，判斷是 MathCAT
  這裡沒有針對比例語境特化、直接落回通用冒號規則。跟 vi-tools 教材（見
  `document/教材/Nemeth-轉寫依據紀錄.md`）交叉驗證時發現，沒有回報上游，只在這裡記錄。
